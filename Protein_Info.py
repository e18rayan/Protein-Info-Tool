...
parser = argparse.ArgumentParser(
        description = ("A CSV parser that takes in the specific file and outputs "
                        "several protein properties including protein sequence, length, "
        ...
        epilog = "Outputs a file with protein information as 'Protein_results.csv' \n"
                "with 'Translation', 'Length', 'Molecular Weight' in g/mol, and "
...
def prot_length(Prot_input):
    #Returns the length of protein. If it has a stop codon, it returns -1 of the length
    if Prot_input.startswith(("Not", "Invalid")):
        return "0"
    elif Prot_input.endswith(("*", "-")):
        return len(Prot_input) - 1
    else:
...
def prot_MW(Prot_input):
    ...
    ##The 18 refers to the weight of water, total, present in N (2 H's) and C (1 O) termini
    prot_mol_weight = 0
    ...
    try:
        ...
        return f"{18 + prot_mol_weight:.2f}"
...
if __name__ == "__main__":
    try:
        ...
        sys.exit ("The file does not exist or is not a CSV file or has incorrect arguments")
            raise FileNotFoundError
